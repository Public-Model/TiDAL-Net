...
import argparse,subprocess,sys,yaml,json,numpy as np
...
raw=yaml.safe_load(Path(corruption_path).read_text())
dataset_name=cfg.get("dataset",{}).get("name") or cfg.get("data",{}).get("dataset_name")
cc=resolve_corruption_config(raw,affected_ratio=a.ratio,dataset=dataset_name); root_out=Path(cfg["training"]["output_dir"])/f"ratio_{a.ratio:.2f}"; root_out.mkdir(parents=True,exist_ok=True)
for i in range(a.runs):
    ...
    chunks=[]; split_meta={}; bounds=chronological_bounds(len(clean["values"]),cfg["data"]["split"])
    for split_idx,(name,(s,e)) in enumerate(bounds.items()):
        ...
        split_meta[name]=json.loads(str(part.pop("metadata_json"))); chunks.append(part)
    corrupted={k:np.concatenate([c[k] for c in chunks],axis=0) for k in chunks[0]}
    corrupted["metadata_json"]=np.asarray(json.dumps({"mode":"split_wise","dataset_profile":dataset_name,"base_seed":seed,"ratio":a.ratio,"split_seed_offsets":[0,1,2],"resolved_config":cc.__dict__,"splits":split_meta}))
